...
from pycram.robot_plans import ParkArmsActionDescription, MoveGripperMotion, NavigateAction, NavigateActionDescription
...
def take_object_from_human(goal_pose: PoseStamped):
    ...
    plan2 = SequentialPlan(context, MoveGripperMotion(motion=GripperState.CLOSE, gripper=Arms.LEFT),
...
def main():
    goal_pose = PoseStamped.from_list(position=[1, 1, 1], orientation=[0, 0, 1, 1], frame=world_root)
